...
def main():
    ...
    parser.add_argument("--test_aspect", default=None, type=str, choices=['sentiment', 'topic'])
    parser.add_argument("--device_num", default=None, type=str)
    args = parser.parse_args()
    # args.device = DEVICE
    args.device = torch.device("cuda:{}".format(args.device_num))    

    aspect_list = ['sentiment', 'topic']
    acc_dic = dict()
    for aspect in aspect_list:
        args.test_aspect = aspect
        # assert args.test_aspect is not None
        if args.test_aspect == 'sentiment':
            args.model_name_or_path = './classifiers/Amazon/sentiment'
            args.tar_dim = 2
        elif args.test_aspect == 'topic':
            args.model_name_or_path = './classifiers/Amazon/topic'
            args.tar_dim = 6
